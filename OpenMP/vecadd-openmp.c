...
void initialise(float ** a, float ** b, float ** c, const int N)
{
  ...
  *a = malloc(sizeof(float) * N);
  if (*a == NULL) die("cannot allocate memory for a", __LINE__, __FILE__);
  *b = malloc(sizeof(float) * N);
  if (*b == NULL) die("cannot allocate memory for b", __LINE__, __FILE__);
  *c = malloc(sizeof(float) * N);
  if (*c == NULL) die("cannot allocate memory for c", __LINE__, __FILE__);
}
