...
int main(int argc, char const *argv[]) {
  ...
  err = clEnqueueWriteBuffer(ocl.queue, ocl.d_a, CL_TRUE, 0, sizeof(float) * N,
                             h_a, 0, NULL, NULL);
  checkError(err, "writing h_a data", __LINE__);

  // Write h_b to OpenCL buffer on the device
  err = clEnqueueWriteBuffer(ocl.queue, ocl.d_b, CL_TRUE, 0, sizeof(float) * N,
                             h_b, 0, NULL, NULL);
  ...
  for (int itr = 0; itr < num_iterations; itr++) {
    ...
    // Enqueue kernel
    size_t global[1] = {N};
    err = clEnqueueNDRangeKernel(ocl.queue, ocl.vecadd, 1, NULL, global, NULL,
                                 0, NULL, NULL);
  ...
  err = clEnqueueReadBuffer(ocl.queue, ocl.d_c, CL_TRUE, 0, sizeof(float) * N,
                            h_c, 0, NULL, NULL);
...
void initialise(t_ocl *ocl, float **h_a, float **h_b, float **h_c,
                const int N) {
  ...
  *h_a = malloc(sizeof(float) * N);
  if (*h_a == NULL)
    die("cannot allocate memory for h_a", __LINE__, __FILE__);
  *h_b = malloc(sizeof(float) * N);
  if (*h_b == NULL)
    die("cannot allocate memory for h_b", __LINE__, __FILE__);
  *h_c = malloc(sizeof(float) * N);
  if (*h_c == NULL)
    die("cannot allocate memory for h_c", __LINE__, __FILE__);
  ...
  ocl->d_a = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY,
                            sizeof(cl_float) * N, NULL, &err);
  checkError(err, "creating buffer a", __LINE__);
  ocl->d_b = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY,
                            sizeof(cl_float) * N, NULL, &err);
  checkError(err, "creating buffer b", __LINE__);
  ocl->d_c = clCreateBuffer(ocl->context, CL_MEM_WRITE_ONLY,
                            sizeof(cl_float) * N, NULL, &err);
